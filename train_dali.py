...
def cli_main():
    ...
    external_input_iterator_train = ExternalCifarInputIterator(cifar_ds=cifar_train, batch_size=args.batch_size)
    external_input_iterator_val = ExternalCifarInputIterator(cifar_ds=cifar_val, batch_size=args.batch_size)
    external_input_iterator_test = ExternalCifarInputIterator(cifar_ds=cifar_test, batch_size=args.batch_size)
    ...
    pipe_train.build()
    train_loader = DALIClassificationLoader(pipe_train, size=len(cifar_train), auto_reset=True, last_batch_policy=False)
    ...
    pipe_val.build()
    val_loader = DALIClassificationLoader(pipe_val, size=len(cifar_val), auto_reset=True, last_batch_policy=False)
    ...
    pipe_test.build()
    test_loader = DALIClassificationLoader(pipe_test, size=len(cifar_test), auto_reset=True, last_batch_policy=False)
    ...
    trainer = pl.Trainer.from_argparse_args(args, callbacks=[model_checkpoint, early_stop_callback], detect_anomaly=True)
