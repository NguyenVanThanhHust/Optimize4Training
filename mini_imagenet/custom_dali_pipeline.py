...
class HybridTrainPipeline(Pipeline):
    ...
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, 
                mean, std, local_rank=0, world_size=1, dali_cpu=False, 
                shuffle=True, fp16=False, min_crop_size=0.08):
        # Set seed = -1 because we recreate pipeline at every epoch
        super(HybridTrainPipeline, self).__init__(batch_size, num_threads, device_id, seed=-1)

        self.input = fn.readers.file(file_root=data_dir, shard_id=local_rank, num_shards=world_size,
...
class HybridValPipe(Pipeline):
    ...
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, size,
        ...
        self.input = fn.readers.file(file_root=data_dir, shard_id=local_rank, num_shards=world_size,
                                     random_shuffle=shuffle, initial_fill=1)
...
class DaliIterator():
    ...
    def __init__(self, pipelines, size, **kwargs):
        self._dali_interator = DALIClassificationIterator(pipelines=pipelines, size=size)
...
def _preproc_worker(dali_iterator, cuda_stream, fp16, )
